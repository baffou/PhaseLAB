    ...
    methods
        ...
        function makeMoviedx(IM,videoName,opt)
            ...
            for u=1:numel(IM)
                ...
                if isempty(opt.zrange)
                    opendx(IM(u),persp=opt.persp,AZ_camera=opt.phi,PO_camera=opt.theta,ampl=opt.ampl,colorMap=opt.colorMap)
                else
                    opendx(IM(u),persp=opt.persp,AZ_camera=opt.phi,PO_camera=opt.theta,ampl=opt.ampl,zrange=opt.zrange,colorMap=opt.colorMap)
                end
                frame=getframe(hfig);
                drawnow
                writeVideo(writerObj, frame);
