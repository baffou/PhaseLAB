...
function [alpha,OV]=magicWandAlphaOV(hfig)
...
for n=1:Nim  % loop on the list of images
    ...
    for iNP=1:NNP  % loop on the list of particles
        ...
        imwrite((OPDn-minOPD)*255/(maxOPD-minOPD),phase1024(256),[folder '/masks/' generateDatedFileName() '_OPDcrop_' IM.OPDfileName '.jpg'])
        imwrite(maskMeas*255,gray(256),[folder '/masks/' generateDatedFileName()  '_Mask_' IM.OPDfileName '.jpg'])
        imwrite(ringBMeas*255,gray(256),[folder '/masks/' generateDatedFileName()  '_RingB_' IM.OPDfileName '.jpg'])
        
        
        %% save the csv file
        % if csv file not already created, write the first line with the row's names.
        if ~exist([folder '/data.csv'],'file') 
            Tab0=table({'file name','time','factor','alpha','OV','XY'});
        ...
        Tab=table({IM.OPDfileName},{date},efactor,{sprintf('=COMPLEX(%.4g,%.4g)',alphaRealMean/efactor,alphaImagMean/efactor)},{sprintf('%.4g',OV(iNP,n)/efactor)},{num2str(posList(:).')});
