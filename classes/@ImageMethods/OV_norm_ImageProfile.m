function [alphaNP,OVNP]=OV_norm_ImageProfile(IM,varargin)
%OV_norm_ImageProfile(IM,hfigInit)
%OV_norm_ImageProfile(IM,nmax)
%OV_norm_ImageProfile(IM,nmax,Dz)
%OV_norm_ImageProfile(IM,nmax,Dz,nbgthick)
    ...
    if nmax0==0  % sum over the whole image
        ...
        while(button~=1) % i.e. as long as it is not a mouse click
            ...
            for co = 1:Noo
                ...
                OV(co)=sum(sqrtT.*Phnormask*IM.lambda/(2*pi))*taillePx.*taillePx;
