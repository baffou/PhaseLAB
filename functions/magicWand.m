...
% Check arguments
if any(ylist > H)
    error('Row cordinates greater than the image height (%g).', H);
end

if any(xlist > W)
    error('Column cordinates greater than the image height (%g).', W);
end
...
for idx = 1:length(ylist)
    ref_r = double(im(ylist(idx), xlist(idx)));
