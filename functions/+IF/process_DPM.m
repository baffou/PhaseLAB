...
h=figure;
imagegb(log(abs(FExg)))
fullscreen
if opt.auto
    [x1,y1]=maxPix(abs(FExg).*double((R02>r1^2)));
else
    [x1,y1]=ginput(1);
end
...
Itf0=abs(Exc).^2+abs(Eyc).^2;
Itf0ref=abs(Excref).^2+abs(Eycref).^2;
...
nc=20;
IMdpm0u=Unwrap_TIE_DCT_Iter(IMdpm0(nc+1:IM.Ny-nc,nc+1:IM.Nx-nc));
...
OPDdpm=PHIdpm*IM.Illumination.lambda/(2*pi);
