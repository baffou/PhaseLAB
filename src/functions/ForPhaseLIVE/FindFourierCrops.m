function [Fcrops] = FindFourierCrops(Itf)
...
if length(Itf) > 1
    warning('List of Interfero as an input - Only the first one is used')
    Itf = Itf(1);
end
...
RappPixelSize = 0.5*Itf.CGcam.CG.Gamma/Itf.CGcam.Camera.dxSize;
mat_fft = fftshift(fft2(Itf.Itf));

%Find Rx, Ry the radius of the ellips, add +/- pixels errors to search in
Rx_in = refineRadius * (Nx / (RappPixelSize * Itf.CGcam.zoom) );
Rx_out = (1/refineRadius) * (Nx / (RappPixelSize * Itf.CGcam.zoom));
Ry_in = refineRadius * (Ny / (RappPixelSize * Itf.CGcam.zoom));
Ry_out = (1/refineRadius) * (Ny / (RappPixelSize * Itf.CGcam.zoom));
...
Rx = Nx * Itf.CGcam.Camera.dxSize / (Itf.CGcam.CG.Gamma * Zoom);
Ry = Ny * Itf.CGcam.Camera.dxSize / (Itf.CGcam.CG.Gamma * Zoom);
