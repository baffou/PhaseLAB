...
c     variables for the incident field and local field
     ...
     $     ,thetam(10),ssm(10),ppm(10)
      ...
      double complex, dimension(3*nxm*nym*nzm) :: FF,FF0,FFloc,FFnl
      double complex, dimension(nxm*nym*nzm) :: FFscal
...
c     variable pour avoir l'image a travers la lentille
     ...
     $     ,Ediffkzneg(nfft2d,nfft2d,3)
      ...
      character(LEN=100) :: h5file,h5loop
...
c     parametre de la boucle
      nmaxloop=4
      h5loop='essai.h5'
      call hdf5create(h5loop, file_id_loop)
...
c     beam='arbitrary'        !Arbitrary wave defines by the user
      ...
      if (beam(1:11).eq.'pwavelinear') then
         theta=30.d0
         phi=180.d0*dble(iloop-1)/dble(nmaxloop)*2.d0
         pp=0.d0
         ss=1.d0
...
c     object='arbitrary'            !Arbitrary object
      ...
      if (object(1:6).eq.'sphere') then
         ...
         pxSize=65.d0
         nnnr=10
         rayonmulti(1)=dble(nnnr)*pxSize         
         xgmulti(1)=0.d0
         ygmulti(1)=0.d0
         zgmulti(1)=-rayonmulti(1)
         materiaumulti(1)='xx'
...
c     save the file
     ...
     $     ,nfft2d,imaxk0,Ediffkzpos,k,name ,group_idim_loop)
      ...
      if (iloop.eq.1) then
         ...
         call hdf5write1d_int(group_idopt_loop,datasetname,aretecube,dim)  
         ...
         datasetname='ndipole'
         call hdf5write(group_idopt_loop,datasetname,pxSize,dim)
         
         datasetname='aretecube'
         call hdf5write(group_idopt_loop,datasetname,pxSize,dim)

         datasetname='epsmax'
         call hdf5write(group_idopt_loop,datasetname,maxval(dreal(epsilon)),dim)

         datasetname='epsmin'
         call hdf5write(group_idopt_loop,datasetname,dreal(epscouche(0)),dim)
