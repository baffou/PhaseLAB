classdef CGcamera  <  handle & matlab.mixin.Copyable
    ...
    methods
                    ...
                    if istext(fileName) % check if it is text, only option.
                        ...
                        if strcmpi(camType,'CGcamera')
                            ...
                            while ~isnumeric(line)
                                ...
                                if strcmp(parameter,'dexel size')
                                    dxSize = str2double(line(colPos+1:end));
                                ...
                                elseif strcmp(parameter,'Ny')
                                    Ny = str2double(line(colPos+1:end));
                                elseif strcmp(parameter,'CGdepth')
                                    CGdepth = str2double(line(colPos+1:end));
                                elseif strcmp(parameter,'CGangle')
                                    CGangle = str2double(line(colPos+1:end));
        ...
        function obj = CGrotate(obj,beta)
            if nargout
                obj  = duplicate(obj);
            end
