...
.. dropdown:: **crop** |subTitle| Crop the images of the object. |/subTitle|
    ...
    ``obj.crop()`` crops the ``T`` and ``OPD`` attribute images, identically. It also crops ``DWx`` and ``DWy`` if they are defined. A figure window opens, inviting the user to click on the image to define a square area centered in the middle of the image.
