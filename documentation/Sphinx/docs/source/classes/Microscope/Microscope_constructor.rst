...
.. dropdown:: **Constructor**
    ...
    ``M`` is the objective magnification, ``OB`` is an *Objective* object  (see :ref:`The_Objective_class`), ``brand`` is the brand of the microscope, ``f_TL`` is the focal length of the tube lens (TL), ``CGcam`` is a *CGcamera* object, ``Cam`` is a *Camera* object and ``software`` is the software used to acquire data (must belong to *{'Sid4Bio','PHAST','CG','PhaseLIVE','other'}*).
